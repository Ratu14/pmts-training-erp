...
function candidateCode(serial: string | number, enrolledAt: string) {
  ...
  const year = /^\d{4}-\d{2}-\d{2}$/.test(enrolledAt) ? enrolledAt.slice(2, 4) : '';
  return Number.isInteger(serialNumber) && serialNumber > 0 && year ? String(serialNumber) + '-' + year : '—';
}
...
async function postERP(payload: Record<string, unknown>) {
  ...
  const result = (await response.json().catch(() => ({}))) as { error?: unknown; id?: unknown; serialNumber?: unknown; enrollmentYear?: unknown };
...
export function TrainingERP() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setCandidates(Array.isArray(payload.candidates) ? payload.candidates.map((item) => {
          const row = item as { id?: unknown; serialNumber?: unknown; enrollmentYear?: unknown; name?: unknown; phone?: unknown; enrolledAt?: unknown };
          return {
            id: String(row.id ?? ''),
            serialNumber: typeof row.serialNumber === 'number' ? row.serialNumber : null,
            enrollmentYear: typeof row.enrollmentYear === 'number' ? row.enrollmentYear : null,
            name: String(row.name ?? ''),
            phone: String(row.phone ?? '—'),
            enrolled: String(row.enrolledAt ?? '—'),
          };
        }) : []);
        setSessions(Array.isArray(payload.sessions) ? payload.sessions.map((item) => {
          const row = item as { id?: unknown; candidateId?: unknown; candidateName?: unknown; sessionDate?: unknown; timeSlot?: unknown; status?: unknown; trainerId?: unknown; trainerName?: unknown; notes?: unknown };
          return {
            id: String(row.id ?? ''),
        ...
        setTrainers(Array.isArray(payload.trainers) ? payload.trainers.map((item) => {
  ...
  const rows: ProgressCandidate[] = useMemo(() => {
    const completedByCandidate = sessions.reduce<Record<string, number>>((counts, session) => {
  ...
  async function createCandidate(input: { name: string; phone: string; serialNumber: string; enrolledAt: string }) {
    ...
    setCandidates((current) => [...current, {
      id,
      serialNumber: typeof result.serialNumber === 'number' ? result.serialNumber : Number(input.serialNumber),
      enrollmentYear: typeof result.enrollmentYear === 'number' ? result.enrollmentYear : Number(input.enrolledAt.slice(2, 4)),
      name: input.name,
      phone: input.phone || '—',
  ...
  async function createSession(input: { candidateId: string; date: string; slot: string; trainerId: string; status: Status; notes: string }) {
    const result = await postERP({ action: 'session', candidateId: input.candidateId, sessionDate: input.date, timeSlot: input.slot, trainerId: input.trainerId, status: input.status, notes: input.notes });
    const id = String(result.id ?? '');
  ...
  async function submitCandidate(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage(null);
    try {
      await createCandidate({ name, phone, serialNumber: candidateSerial, enrolledAt: enrollmentDate });
      setAddOpen(false);
      setCandidateSerial('');
  ...
  async function completeNext() {
    ...
    setMessage(null);
    try {
      await postERP({ action: 'session-status', id: next.id, status: 'Completed' });
      setSessions((current) => current.map((session) => session.id === next.id ? { ...session, status: 'Completed' } : session));
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Unable to update the session.');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function saveTarget(value: number) {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : 'Unable to save the training target.');
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fc] text-[#151724]">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-[246px] flex-col border-r border-[#e5e7f0] bg-white px-4 py-5 lg:flex">
        ...
        <nav className="mt-10 space-y-1" aria-label="Primary navigation">{navItems.map((item) => {
          const active = page.href === item.href;
          return <a key={item.href} href={item.href} className={'flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-colors ' + (active ? 'bg-[#ececff] font-medium text-[#363681]' : 'text-[#686c80] hover:bg-[#f4f5f9] hover:text-[#25255e]')}><Icon icon={item.icon} size={18} />{item.label}</a>;
        })}</nav>
      ...
      <main className="lg:pl-[246px]">
        ...
        <div className="mx-auto max-w-[1500px] px-5 py-7 sm:px-8 lg:px-10">
          ...
          {loaded && page.href === '/' && <Dashboard candidate={selected} candidates={rows} sessions={sessions} target={target} onAdd={() => setAddOpen(true)} />}
          ...
          {loaded && page.href === '/training-log' && <TrainingLog candidates={rows} sessions={sessions} trainers={trainers} saving={saving} onCreate={createSession} onComplete={completeNext} />}
          {loaded && page.href === '/reports' && <Reports candidates={rows} completed={completed} noShows={noShows} attendance={attendance} target={target} />}
          {loaded && page.href === '/settings' && <Settings target={target} trainers={trainers} saving={saving} onChange={setTarget} onSave={saveTarget} />}
        </div>
      </main>

      {addOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-[#151724]/30 p-4 backdrop-blur-sm"><Card className="w-full max-w-md bg-white shadow-2xl"><CardHeader><CardTitle className="text-lg">Add a learner</CardTitle><CardDescription>Create a candidate record in the PMTS database.</CardDescription></CardHeader><CardContent><form className="space-y-4" onSubmit={submitCandidate}><label className="block text-sm font-medium">Learner name<Input required name="name" placeholder="e.g. Priya Banerjee" className="mt-1.5" /></label><label className="block text-sm font-medium">Phone number<Input name="phone" placeholder="e.g. 9876543210" className="mt-1.5" /></label><div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-medium">Candidate serial number<Input required type="number" min="1" step="1" value={candidateSerial} onChange={(event) => setCandidateSerial(event.target.value)} placeholder="e.g. 82" className="mt-1.5 font-mono" /></label><label className="block text-sm font-medium">Enrollment date<Input required type="date" value={enrollmentDate} onChange={(event) => setEnrollmentDate(event.target.value)} className="mt-1.5" /></label></div><div className="rounded-xl border border-[#dfe2ec] bg-[#fafbfe] px-3 py-2.5"><p className="font-mono text-[10px] uppercase tracking-[0.15em] text-[#85899b]">Candidate ID preview</p><p className="mt-1 font-mono text-sm font-semibold text-[#41418e]">{candidateCode(candidateSerial, enrollmentDate)}</p></div><div className="flex justify-end gap-2 pt-2"><Button type="button" variant="outline" onClick={() => setAddOpen(false)}>Cancel</Button><Button type="submit" disabled={saving}><Icon icon={UserAdd02Icon} /> {saving ? 'Saving…' : 'Create learner'}</Button></div></form></CardContent></Card></div>}
    </div>
  );
}

function Dashboard({ candidate, candidates, sessions, target, onAdd }: { candidate?: ProgressCandidate; candidates: ProgressCandidate[]; sessions: Session[]; target: number; onAdd: () => void }) {
  if (!candidate) return <Card className="border border-dashed border-[#cfd2e4] bg-white"><CardContent className="flex min-h-80 flex-col items-center justify-center text-center"><div className="grid size-12 place-items-center rounded-2xl bg-[#ececff] text-[#3f3f91]"><Icon icon={UserAdd02Icon} size={22} /></div><p className="mt-5 font-mono text-[11px] uppercase tracking-[0.16em] text-[#777b91]">Database ready</p><h2 className="mt-2 text-2xl font-semibold tracking-[-0.04em]">Add your first learner</h2><p className="mt-2 max-w-md text-sm leading-6 text-[#686d81]">New learners and training sessions will be saved to your Cloudflare D1 database.</p><Button className="mt-6" onClick={onAdd}><Icon icon={UserAdd02Icon} /> Add learner</Button></CardContent></Card>;
...
function Candidates({ candidates, target, search, onSearch, onAdd }: { candidates: ProgressCandidate[]; target: number; search: string; onSearch: (value: string) => void; onAdd: () => void }) {
  const filtered = candidates.filter((candidate) => (candidate.name + ' ' + candidate.id + ' ' + candidate.phone).toLowerCase().includes(search.toLowerCase()));
  return <><section className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="font-mono text-[11px] uppercase tracking-[0.16em] text-[#747991]">Master data</p><h2 className="mt-2 text-3xl font-semibold tracking-[-0.05em]">Candidate register</h2><p className="mt-2 text-sm text-[#686d81]">Each learner has a serial-year ID and an automatically calculated completion pace.</p></div><Button onClick={onAdd}><Icon icon={UserAdd02Icon} /> Add learner</Button></section><Card className="bg-white"><CardHeader><div className="relative w-full max-w-sm"><Icon icon={Search02Icon} size={17} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#85899b]" /><Input value={search} onChange={(event) => onSearch(event.target.value)} className="pl-9" placeholder="Search ID, learner, or phone" /></div><Badge variant="outline">{filtered.length} records</Badge></CardHeader><CardContent>{filtered.length ? <Table><TableHeader><TableRow><TableHead>Learner</TableHead><TableHead>Candidate ID</TableHead><TableHead className="hidden md:table-cell">Enrollment</TableHead><TableHead className="hidden lg:table-cell">Contact</TableHead><TableHead>Progress</TableHead><TableHead className="text-right">Remaining</TableHead></TableRow></TableHeader><TableBody>{filtered.map((candidate) => <TableRow key={candidate.id}><TableCell><a href={'/?candidate=' + encodeURIComponent(candidate.id)} className="block hover:text-[#4b4b9d]"><p className="font-medium">{candidate.name}</p><p className="font-mono text-[10px] text-[#85899b]">Record</p></a></TableCell><TableCell className="font-mono text-xs text-[#5559a8]">{candidate.id}</TableCell><TableCell className="hidden text-[#73788d] md:table-cell">{candidate.enrolled}</TableCell><TableCell className="hidden font-mono text-xs text-[#73788d] lg:table-cell">{candidate.phone}</TableCell><TableCell className="min-w-[150px]"><div className="flex items-center gap-3"><Progress value={candidate.percentage} className="flex-1"><ProgressLabel className="sr-only">{candidate.name} completion</ProgressLabel><ProgressValue className="sr-only" /></Progress><span className="font-mono text-[11px] text-[#5559a8]">{candidate.complete}/{target}</span></div></TableCell><TableCell className="text-right font-mono text-xs text-[#5559a8]">{candidate.remaining}</TableCell></TableRow>)}</TableBody></Table> : <p className="py-10 text-center text-sm text-[#73788d]">No learners yet. Add the first learner to create a D1 record.</p>}</CardContent></Card></>;
}

function TrainingLog({ candidates, sessions, trainers, saving, onCreate, onComplete }: { candidates: ProgressCandidate[]; sessions: Session[]; trainers: Trainer[]; saving: boolean; onCreate: (input: { candidateId: string; date: string; slot: string; trainerId: string; status: Status; notes: string }) => Promise<void>; onComplete: () => Promise<void> }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!trainerId) { setError('Choose a trainer before saving the session.'); return; }
    setError(null);
    try {
...
function Reports({ candidates, completed, noShows, attendance, target }: { candidates: ProgressCandidate[]; completed: number; noShows: number; attendance: number; target: number }) {
  ...
  return <><section className="mb-7"><p className="font-mono text-[11px] uppercase tracking-[0.16em] text-[#747991]">Operations intelligence</p><h2 className="mt-2 text-3xl font-semibold tracking-[-0.05em]">Training performance</h2><p className="mt-2 text-sm text-[#686d81]">A simple performance lens for the team—not another spreadsheet to maintain.</p></section><div className="grid gap-4 md:grid-cols-3"><Metric label="Completed sessions" value={String(completed)} detail={'of ' + String(totalTarget) + ' required'} /><Metric label="Attendance rate" value={String(attendance) + '%'} detail="From recorded outcomes" /><Metric label="No-show records" value={String(noShows)} detail={'Portfolio completion: ' + String(portfolio) + '%'} /></div></>;
}

function Settings({ target, trainers, saving, onChange, onSave }: { target: number; trainers: Trainer[]; saving: boolean; onChange: (value: number) => void; onSave: (value: number) => Promise<void> }) {
  const slots = ['07:00', '07:30', '08:00', '08:30', '09:00', '09:30', '10:00', '10:30', '11:00', '11:30', '15:30', '16:00'];
  return <><section className="mb-7"><p className="font-mono text-[11px] uppercase tracking-[0.16em] text-[#747991]">System configuration</p><h2 className="mt-2 text-3xl font-semibold tracking-[-0.05em]">Training rules</h2><p className="mt-2 text-sm text-[#686d81]">Set the operational standard that drives candidate progress and session availability.</p></section><div className="grid gap-6 xl:grid-cols-3"><Card className="bg-white"><CardHeader><div><CardTitle>Completion target</CardTitle><CardDescription>Required sessions per learner.</CardDescription></div></CardHeader><CardContent><label className="text-sm font-medium">Sessions required<Input type="number" min="1" value={target} onChange={(event) => onChange(Math.max(1, Number(event.target.value) || 1))} onBlur={() => void onSave(target)} className="mt-2 max-w-44 font-mono text-base" disabled={saving} /></label><p className="mt-4 text-sm leading-6 text-[#73788d]">This value is saved in D1 and updates calculations throughout the application.</p></CardContent></Card><Card className="bg-white"><CardHeader><div><CardTitle>Trainer directory</CardTitle><CardDescription>Active records populate the session form.</CardDescription></div><Badge variant="outline">{trainers.length} active</Badge></CardHeader><CardContent>{trainers.length ? <div className="space-y-2">{trainers.map((trainer) => <div key={trainer.id} className="flex items-center justify-between rounded-xl border border-[#e5e7f0] bg-[#fafbfe] px-3 py-2.5"><span className="text-sm font-medium">{trainer.name}</span><span className="font-mono text-[10px] text-[#85899b]">Trainer</span></div>)}</div> : <p className="text-sm text-[#73788d]">Loading trainer records…</p>}<p className="mt-4 text-xs leading-5 text-[#73788d]">Administrator-only changes to the trainer table will be added with protected access later.</p></CardContent></Card><Card className="bg-white"><CardHeader><div><CardTitle>Available time slots</CardTitle><CardDescription>Suggested windows for training sessions.</CardDescription></div></CardHeader><CardContent><div className="grid grid-cols-3 gap-2">{slots.map((slot) => <div key={slot} className="rounded-lg border border-[#e5e7f0] bg-[#fafbfe] px-3 py-2 font-mono text-xs text-[#5559a8]">{slot}</div>)}</div><div className="mt-6 border-t border-[#e5e7f0] pt-5"><p className="font-mono text-[10px] uppercase tracking-[0.15em] text-[#85899b]">Session statuses</p><div className="mt-3 flex flex-wrap gap-2">{STATUSES.map((status) => <Badge key={status} className={statusStyle(status) + ' ring-1'}>{status}</Badge>)}</div></div></CardContent></Card></div></>;
}
